...
if check == 0:
	...
	check = vip.SI_discount_check()

	if check == 0:

		check = vip.login_process()
		vip.merchandise()
		vip.goods_num()

		if check == 0:

			check = vip.M_discount_check()
...
# 3. 구매하기 버튼 선택
check = checkout.address_change()
if check == 0:

	card_random = checkout.payment_method()
	check = checkout.card_halin()

	if check == 0:

		check = checkout.smi_cash()

		if check == 0:

			check = checkout.last_buy()

			if check == 0:

				check = checkout.payment(card_random)

				if check == 0:

					che = order_complete.order_completion()
